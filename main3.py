...
MIN_BOT_SPEED = int(PLAYER_SPEED * 0.4)  # Minimum bot speed at 0.4x player speed
MAX_BOT_SPEED = int(PLAYER_SPEED * 1.1)  # Max bot speed limited to 1.1x player speed
...
for i in range(BOT_COUNT):
    ...
    bot_speeds.append(random.randint(MIN_BOT_SPEED, MAX_BOT_SPEED))  # Random speeds for bots, limited within range
...
while running:
    ...
    for i, bot in enumerate(bots):
        if green_light:
            ...
            if not bot_states[i] and random.random() < BOT_DEATH_CHANCE:  # More varied deaths
